...
This document covers all keyboard shortcuts in MikMik, how to customize them, vim mode, and special input behaviors.
...
| `Ctrl+D`       | exit           | Exit MikMik (non-rebindable)                           |
...
These bindings are active when MikMik is displaying a yes/no confirmation prompt (e.g., tool permission requests).
...
MikMik uses a context system so that the same key can have different effects depending on where focus is. A binding in a more specific context takes precedence over a binding in a broader context.
...
`keybindings.json` carries a top-level `schema_version` field (currently `1`). When MikMik's defaults change in a release, the file is auto-migrated on next launch:

1. MikMik reads the file and compares `schema_version` against the bundled `KEYBINDINGS_SCHEMA_VERSION`.
2. If the file is older, MikMik runs a **smart merge**:
...
After editing the file, run `/keybindings` and then exit to trigger a reload, or restart MikMik.

### Chord Bindings

MikMik supports chord bindings — multi-key sequences where you press a leader key and then a follow-up key. Chord bindings are defined with a `chord` array instead of a single `key`:
...
| `Ctrl+D` | Exit MikMik when input is empty; signal EOF when input has content       |
| `Ctrl+M` | Identical to `Enter` at the terminal level (terminals emit `CR` for both) |

These keys are handled at the terminal input layer before the keybinding system processes events. If any of them appear as a `chord` in `keybindings.json`, MikMik:
...
When nothing answers, MikMik says so and what to do about it. Over SSH the usual fix is to set `mouseCapture` to `false` (see [Configuration](configuration.md#interface)) so the terminal's own paste works again.

---

### @file Injection with Typeahead

Type `@` followed by a path in the prompt to inject a file's contents into your message. The `@` token only triggers when it is at a word boundary (start of input or preceded by whitespace). As you type after the `@`, MikMik opens a typeahead completion overlay scanning the current working directory.
...
**Limits and warnings.** If a referenced path is too large, binary, a directory, or unreadable, MikMik opens a confirmation dialog before sending:
...
On non-English keyboard layouts (Cyrillic, Arabic, Greek, CJK, etc.), the Latin letters used in MikMik's shortcuts may not appear on the keycaps, and some input methods send layout-translated scan codes for Ctrl combinations — causing MikMik to miss the shortcut entirely.

### The Fix

MikMik resolves this by mapping `Ctrl+<scancode>` events to their QWERTY positional equivalents before keybinding lookup. Concretely:
...
On macOS, `Alt` (Option) key combinations produce special Unicode characters at the OS level before they reach the terminal. MikMik intercepts these at the terminal input layer and re-emits them as `alt+<key>` events using the same positional mapping described above.
