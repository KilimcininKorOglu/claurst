...
struct ModelSyncOutcome {
    ...
    overrides: Vec<(String, claurst_core::config::ModelOverride)>,
}
...
fn apply_model_sync(config: &mut Config, account_id: &str, outcome: &ModelSyncOutcome) {
    let entry = config
        .provider_configs
...
fn persist_account_models(
    account_id: &str,
    models: &[claurst_api::ModelInfo],
    force: bool,
    ...
    for model in models {
        ...
        let value = claurst_core::config::ModelOverride {
...
/// replaces it.
fn describe_model_sync(account_id: &str, outcome: &ModelSyncOutcome) -> String {
            ...
            match provider {
                Some(provider) => match provider.discover_models().await {
                    Ok(models) if !models.is_empty() => {
                        match persist_account_models(&account_id, &models, force) {
                            Ok(outcome) => {
                                ...
                                apply_model_sync(&mut app.config, &account_id, &outcome);
                                apply_model_sync(&mut cmd_ctx.config, &account_id, &outcome);
                                apply_model_sync(&mut tool_ctx.config, &account_id, &outcome);
                                app.status_message =
                                    Some(describe_model_sync(&account_id, &outcome))
                            }
                            Err(err) => {
                                app.status_message =
