        ...
        for field in &settings.fields {
            let action_style = match field.action {
                ...
                PreviewAction::Replace => Style::default()
                    .fg(MIKMIK_ACCENT)
                    .add_modifier(Modifier::BOLD),
                PreviewAction::Keep => Style::default().fg(MIKMIK_MUTED),
                PreviewAction::Skip => Style::default().fg(MIKMIK_MUTED),
            };
            let mut spans = vec![
                Span::styled(format!("  [{}] ", field.action.label()), action_style),
                Span::styled(field.name.clone(), Style::default().fg(MIKMIK_TEXT)),
            ];
...
fn path_row(label: &str, value: &str) -> Line<'static> {
    Line::from(vec![
        Span::styled(format!("  {}: ", label), Style::default().fg(MIKMIK_MUTED)),
        Span::styled(value.to_string(), Style::default().fg(MIKMIK_TEXT)),
    ])
}
