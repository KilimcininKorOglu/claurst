...
///
/// Reads `MIKMIK_FEATURE_<NORMALIZED_NAME>` and returns `true` when the
...
pub fn is_feature_enabled(gate_name: &str) -> bool {
    let key = format!("MIKMIK_FEATURE_{}", normalize_name(gate_name));
...
///
/// Reads `MIKMIK_DYNAMIC_CONFIG_<NORMALIZED_NAME>`.  If the variable is
...
pub fn get_dynamic_config<T: DeserializeOwned>(name: &str, default: T) -> T {
    let key = format!("MIKMIK_DYNAMIC_CONFIG_{}", normalize_name(name));
...
/// Return `true` when MikMik should run in "bare" (minimal) mode.
///
/// Bare mode skips LSP, plugin, and MCP startup for a faster, lighter
/// experience.  It is enabled by either:
///   - The `MIKMIK_SIMPLE=1` environment variable, OR
///   - The `--bare` flag in `std::env::args()`.
pub fn is_bare_mode() -> bool {
    // Check env var
    if is_env_truthy(std::env::var("MIKMIK_SIMPLE").ok().as_deref()) {
        return true;
    }
