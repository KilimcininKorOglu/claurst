// share_export: Render a MikMik session as a single self-contained HTML file
...
// `https://kilimcininkoroglu.github.io/mikmik/session/#<gist-id>` (overridable via
// `MIKMIK_SHARE_VIEWER_URL`).
...
/// Default viewer base URL used when `MIKMIK_SHARE_VIEWER_URL` is unset.
pub const DEFAULT_SHARE_VIEWER_URL: &str = "https://kilimcininkoroglu.github.io/mikmik/session/";

/// Environment variable that overrides the share viewer base URL.
pub const ENV_SHARE_VIEWER_URL: &str = "MIKMIK_SHARE_VIEWER_URL";
...
/// Build the viewer URL for a given gist id, honoring `MIKMIK_SHARE_VIEWER_URL`.
...
mod tests {
    ...
    fn viewer_url_default_and_override() {
        ...
        assert_eq!(
            ...
            "https://kilimcininkoroglu.github.io/mikmik/session/#deadbeef"
        );
